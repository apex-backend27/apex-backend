...
app.post('/api/user/game/prize', authenticate, async (req,res)=>{
  ...
  const client=await pool.connect();
  try{await client.query('BEGIN'); const q=await client.query(`SELECT * FROM users WHERE id=$1 FOR UPDATE`,[req.userId]); if(!q.rows.length) throw new Error('Usuario no encontrado'); const u=q.rows[0]; const cq=await client.query('SELECT juegos_config FROM configuracion WHERE id=1'); const cfg=(cq.rows[0]&&cq.rows[0].juegos_config)||{}; const rawConfigured=cfg[game]??cfg[prizeField]??(cfg.premios&&cfg.premios[game])??0; const configured=Array.isArray(rawConfigured)?(rawConfigured.map(Number).find(Number.isFinite)||0):rawConfigured&&typeof rawConfigured==='object'?(rawConfigured.valor??rawConfigured.premio??rawConfigured.monto??rawConfigured.amount??(Array.isArray(rawConfigured.premios)?(rawConfigured.premios.map(Number).find(Number.isFinite)||0):0)):rawConfigured; const amount=Number(u[prizeField]||configured||0); const usos=Number(u[usage]||0); if(amount<=0) {await client.query('ROLLBACK');return res.status(409).json({error:'Esta actividad no tiene un premio configurado por el administrador'});} if(usos<=0) {await client.query('ROLLBACK');return res.status(409).json({error:'No tienes usos disponibles'});} const nombresJuego={ruleta:'Máquina de bolas',cofre:'Cofre del tesoro',dados:'Dados'}; const nombreJuego=nombresJuego[game]||game; const item={tipo:'juego',juego:game,actividad:nombreJuego,concepto:`Premio de ${nombreJuego}`,monto:amount,fecha:new Date().toISOString(),estado:'acreditado'}; const hist=Array.isArray(u.historial_detallado)?u.historial_detallado:[]; const hoy=normalizarFechaLima(new Date()); const inicioSemana=normalizarFechaLima(new Date(Date.now()-((new Date().getDay()+6)%7)*86400000)); const semanalInicio=normalizarFechaLima(u.ganado_semanal_inicio)||inicioSemana; const semanal=semanalInicio<inicioSemana?amount:Number(u.ganado_semanal||0)+amount; const out=await client.query(`UPDATE users SET balance=COALESCE(balance,0)+$1, ${usage}=GREATEST(COALESCE(${usage},0)-1,0), total_ganado=COALESCE(total_ganado,0)+$1, ganado_semanal=CASE WHEN COALESCE(ganado_semanal_inicio,DATE '1900-01-01')<DATE_TRUNC('week',CURRENT_TIMESTAMP AT TIME ZONE 'America/Lima')::date THEN $1 ELSE COALESCE(ganado_semanal,0)+$1 END, ganado_semanal_inicio=DATE_TRUNC('week',CURRENT_TIMESTAMP AT TIME ZONE 'America/Lima')::date, historial_detallado=$4::jsonb WHERE id=$5 RETURNING *`,[amount,semanal,inicioSemana,JSON.stringify(hist.concat(item)),req.userId]); await client.query('COMMIT'); try { await crearNotificacion({ userId: req.userId, tipo: 'actividad', titulo: 'Premio acreditado', descripcion: `Ganaste ${amount.toFixed(2)} USDT en ${game}`, accion: 'informativa', entidadId: `${game}:${new Date().toISOString().slice(0,10)}:${Date.now()}`, metadata: { game, amount } }); } catch (notificationError) { console.error('No se pudo crear notificación de premio:', notificationError.message); } res.json({message:'Premio acreditado',premio:amount,user:publicUserData(out.rows[0])});}catch(e){try{await client.query('ROLLBACK')}catch(_){} console.error('game prize',e);res.status(500).json({error:'No se pudo acreditar el premio'})}finally{client.release()}
});
